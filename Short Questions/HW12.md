...
1. GitHub PR → Webhook → Jenkins.
2. Jenkins → `mvn clean install test sonar:sonar`.
3. If SonarQube ≥ 80% → build Docker image + push to registry.
4. Jenkins updates K8s manifest repo with new image tag.
5. ArgoCD auto-syncs repo → deploys to K8s cluster.
6. QA validates in staging. Optionally automated TestNG regression suite.
7. On approval → promote to production.
