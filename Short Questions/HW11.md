...
### 1. Server generates and stores token:
When a user **first accesse**s the application, Spring Security generates a unique CSRF token and stores it in the user’s **HTTP session** (**in-memory by default**) or another configured storage (e.g., Redis).

### 2. Server responses to client with token:
- In traditional web applications (e.g., WordPress, JSP/Servlet-based apps, or PHP sites), the CSRF token is embedded in forms as a hidden input field.
- In SPAs (Single-Page Applications) or applications using AJAX (Asynchronous JavaScript and XML), it is typically sent via a **cookie** or **response header**.

### 3. Client stores and returns token:
- The client stores the token (e.g., in **memory**, a **cookie**, or in the page’s form).
- On subsequent **state-changing** requests (POST, PUT, DELETE, PATCH), the client sends the token back to the server.
- The token is usually included as a **request parameter** or in a **header** (`X-CSRF-TOKEN`).

### 4. Server validates token:
- Spring Security **compares** the token received from the client with the token stored in the session.
- If the tokens match, the request is allowed; otherwise, the request is rejected with **403 Forbidden**.
- Requests using **safe HTTP methods** (GET, HEAD, OPTIONS, TRACE) are **not checked** for CSRF tokens, as they are considered non-state-changing.
