...
1. [K8s Infrastructure](#question-1)
2. [K8s Primitives](#question-2)
3. [Service Types](#question-3)
4. [Exposing Services](#question-4)
5. [Auto-scaling](#question-5)
6. [Handling Node Failures](#question-6)
7. [Node Failure Alert Implementations](#question-7)
8. [Internal DNS](#question-8)
9. [Readiness Probe vs. Liveness Probe](#question-9)
10. [Canary Deployment](#question-10)
...
|----------------|------------------------------------------------------|---------------------------------------|
| Purpose        | Determines if Pod can **receive traffic**            | Determines if Pod needs **restart**   |
| Failure Impact | Pod **removed** from Service endpoints               | Container **restarted**               |
| Use Case       | Slow-starting services, **temporary unavailability** | Detect **hung** or **dead** processes |
| Probe Types    | HTTP GET, TCP Socket, Exec                           | HTTP GET, TCP Socket, Exec            |

<details>
<summary>Example Deployment Manifest</summary>
...
spec:
  ...
  template:
    metadata:
      labels:
        app: my-app  # Labels applied to Pods created by this Deployment
    spec:
      containers:
        - name: my-app-container  # Container name
          ...
          readinessProbe:
            httpGet:
              path: /health  # HTTP endpoint used to check readiness
              port: 8080
          ...
          livenessProbe:
            httpGet:
              path: /health  # HTTP endpoint used to check liveness
              port: 8080
...
Canary deployment is a **delivery strategy** where a new release is first rolled out to a **small subset of users** and then **gradually expanded to all**,
with the ability to **quickly roll back** if issues occur.

### Setup
| Component / Tool      | Purpose                                                                             |
|-----------------------|-------------------------------------------------------------------------------------|
| Deployments & Service | 2 Deployments + 1 Service; traffic split by pod replicas **if no Istio**            |
| Monitoring            | Prometheus + Grafana to track system and canary-specific metrics                    |
| Traffic Control       | **Istio** enables fine-grained routing and weighted traffic distribution            |
| Automation            | **Flagger** automates progressive rollout, promotion, and rollback based on metrics |

### Canary Specific Metrics
- **Error Rate** – HTTP 5xx responses, failed requests, or application errors
- **Latency / Response** Time – p95 or p99 response times
- **Request Throughput** – number of requests per second handled by canary pods
- **CPU Usage** – per canary pod
- **Memory Usage** – per canary pod
- **Pod Restarts / Crashes** – stability of canary pods
- **Business KPIs** – e.g., orders processed, payments succeeded, or feature-specific metrics

### Automated Promotion

**Automated Promotion Logic:**
1. Deploy canary to a **small portion** of traffic (e.g., 10%).
2. Continuously **monitor** metrics against SLOs and/or stable release metrics.
3. Evaluate metrics over a fixed **observation window** (e.g., 5 minutes).
4. Decision rules:
   - **Pass:** Metrics meet SLO thresholds → increase traffic **incrementally** (e.g., 10% → 20%).
   - **Fail:** Metrics violate SLO thresholds → rollback canary immediately.
...
**Response Time–based Promotion Example:**
- Canary receives 10% of traffic.
- Metrics collected over 5 minutes:
  - Stable p95 latency = 250 ms
  - Canary p95 latency = 270 ms
  - Threshold: Canary latency must be within +10% of stable latency
- **Result:** Canary passes → increase traffic to 25%.
- **Rollback:** If at any step, canary latency exceeds the threshold (e.g., > 275 ms), automated rollback occurs.

While canary latency may be slightly higher than the stable version, it may represent a new build with added features under evaluation. Therefore, the SLO focuses on latency difference rather than absolute superiority.